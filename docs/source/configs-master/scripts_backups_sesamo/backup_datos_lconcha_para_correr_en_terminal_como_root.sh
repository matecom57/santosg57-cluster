...
do
  ...
  echo ""
  logfile=/volume1/fmrilab/backup/logs/semi-manual_backup_`basename $thisFolder`_`date '+%F_at_%Hh_%Mmin'`.txt
  ...
  printf "%s %s"  $thisHost $logfile | tee -a $currentRunningFile
   ...
   rsync --progress --stats -h --force --ignore-errors --delete-excluded \
   --include="*.testDir" \
